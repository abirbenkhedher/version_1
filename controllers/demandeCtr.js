...
exports.GetAllDemande = async (req,res)=>{
    try{
      ...
      res.status(400).send({message: `can not get Demande `})
    }
  }
  ...
  exports.GetDemandeById = async (req,res)=>{
    try {
      const result= await Demande.findOne({_id:req.params.id});
      res.send({response: result, message: 'getting Demande successfully'})
    } catch (error) {
      res.status(400).send({message: `can not get Demande with id ${req.params.id} `})
    }
  }
